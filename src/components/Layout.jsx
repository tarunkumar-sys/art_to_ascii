...
const Layout = ({ sidebar, viewer, bottomPanel, exportDropdown, shortcutsDropdown, onHelpClick }) => {
  return (
    <div className="flex h-screen w-full bg-blender-bg text-blender-text font-sans overflow-hidden select-none text-[11px]">
      ...
      {/* Blender Top Bar */}
      <div className="absolute top-0 left-0 right-0 h-6 bg-blender-header border-b border-blender-border flex items-center px-1 z-30 shadow-md whitespace-nowrap">
        {/* Editor Type Selector */}
        <div className="flex items-center px-2 py-0.5 hover:bg-blender-hover rounded-sm cursor-pointer mr-2 transition-colors shrink-0">
          <Monitor className="w-3 h-3 text-blender-active mr-1.5" />
          <ChevronDown className="w-2.5 h-2.5 text-gray-500" />
        </div>

        {/* Menus */}
        <div className="flex items-center gap-0.5 shrink-0 flex-wrap sm:flex-nowrap h-full overflow-visible">
          {['File', 'Edit', 'Render'].map((menu) => (
            <div
              key={menu}
              className="px-2.5 py-0.5 hover:bg-blender-hover rounded-sm cursor-pointer text-gray-300 hover:text-white transition-colors hidden sm:block"
            >
              {menu}
            </div>
          ))}
      ...
      <div className="flex flex-col md:flex-row w-full h-full pt-6">
        {/* 3D/Viewer Viewport */}
        ...
        <div className="w-full h-1/2 md:w-[280px] md:h-full bg-blender-panel border-t md:border-t-0 md:border-l border-blender-border flex flex-col z-10 flex-shrink-0 shadow-xl">
          {/* Header for Properties panel */}
          <div className="h-6 bg-blender-header border-b border-blender-border flex items-center px-2 gap-2 shrink-0">
