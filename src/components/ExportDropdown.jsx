...
const ExportDropdown = ({ asciiOutput }) => {
  ...
  const SectionHeader = ({ label, active = true }) => (
    <div className="flex items-center gap-2 py-1 mb-1 border-b border-white/5">
      <div className={`w-2.5 h-2.5 rounded-[1px] flex items-center justify-center border ${
        active ? 'bg-blender-active border-blue-400/50' : 'bg-blender-input border-blender-border'
      }`}>
  ...
  const ActionButton = ({ label, icon: Icon, onClick, active = false }) => (
    <button
      ...
      className={`flex items-center gap-2 px-2 py-1 rounded-[2px] transition-colors text-left group ${
        active ? 'bg-blender-active text-white' : 'hover:bg-blender-hover text-gray-400 hover:text-gray-200'
      }`}
    >
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`px-2.5 py-0.5 rounded-sm cursor-pointer transition-colors text-[11px] outline-none ${
          isOpen 
